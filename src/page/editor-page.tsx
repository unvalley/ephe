...
import { useMarkdownEditor } from "../features/editor/codemirror/use-markdown-editor";

export const EditorPage = () => {
  const { paperModeClass, paperMode, cyclePaperMode } = usePaperMode();
  const { editorWidth, toggleEditorWidth } = useEditorWidth();
  const { isCommandMenuOpen, toggleCommandMenu } = useCommandK();
  const { editor, content, formatDocument, insertText } = useMarkdownEditor();

  return (
    <div className={`flex h-screen flex-col overflow-hidden antialiased ${paperModeClass}`}>
      <div className="relative flex flex-1 overflow-hidden">
        <div className="z-0 flex-1">
          <CodeMirrorEditor editorRef={editor} />
        </div>
      </div>
