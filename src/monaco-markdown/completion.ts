...
import { type CancellationToken, type editor, languages, type Position as _Position } from "monaco-editor";
...
const completionList = (items: languages.CompletionItem[]): languages.CompletionList => {
  return { suggestions: items.map((v, _) => Object.assign({}, v)) };
};

const newCompletionItem = (text: string, kind: languages.CompletionItemKind): languages.CompletionItem => {
  return {
    label: text,
    kind: kind,
...
class MdCompletionItemProvider implements languages.CompletionItemProvider {
  ...
  constructor() {
    ...
    const c1 = latex._c1.map((cmd) => {
      const item = newCompletionItem(`\\${cmd}`, languages.CompletionItemKind.Function);
      item.insertText = cmd;
      return item;
    });
    // \cmd{$1}
    ...
    const envSnippet = newCompletionItem("\\begin", languages.CompletionItemKind.Snippet);
      ...
      return new Promise((res, _) => {
        ...
        const refLabels = lines.reduce((prev, curr) => {
          ...
          if ((match = /^\[([^\]]*?)\]: (\S*)( .*)?/.exec(curr)) !== null) {
            const ref = match[1];
            const item = newCompletionItem(ref, languages.CompletionItemKind.Reference);
      ...
      return new Promise((res, _) => {
        ...
        const headingCompletions: languages.CompletionItem[] = toc.reduce(
          (prev: languages.CompletionItem[], curr: any) => {
            const item = newCompletionItem(`#${slugify(curr.text)}`, languages.CompletionItemKind.Reference);
